...
from src.cli.templates.evaluation_metrics import (
    get_task_metrics,
    get_tuning_config, 
    get_primary_metric,
...
class TestRegistryEvaluationMetrics:
    ...
    @patch('src.cli.templates.evaluation_metrics.re.findall')
    @patch('src.cli.templates.evaluation_metrics.inspect.getsource')
    def test_extract_metrics_fallback_to_dummy_call(self, mock_getsource, mock_findall):
        ...
        # When
        with patch('src.cli.templates.evaluation_metrics._get_metrics_by_dummy_call') as mock_dummy:
    ...
    @patch('src.cli.templates.evaluation_metrics._extract_metrics_from_evaluator')
    def test_get_task_metrics_with_fallback(self, mock_extract):
        ...
        # Then: fallback 메트릭이 반환되어야 함
        expected_fallback = ['accuracy', 'precision', 'recall', 'f1_score']
