...
from src.cli.utils.system_checker import SystemChecker, CheckStatus, CheckResult
...
class TestSystemChecker:
    ...
    def test_check_data_source_bigquery(self, basic_config):
        ...
        assert result.status == CheckStatus.SUCCESS
        assert "BigQuery dataset accessible" in result.message
    ...
    def test_check_feature_store_none(self, basic_config):
        ...
        assert result.status == CheckStatus.SKIPPED
        assert "Unknown feature store provider" in result.message
    ...
    def test_check_artifact_store_local(self, basic_config):
        """Local Artifact Store 체크."""
        checker = SystemChecker(basic_config, "test")
        
        with patch.object(Path, 'exists', return_value=True):
            result = checker.check_artifact_store(basic_config["artifact_store"])
            
        assert result.status == CheckStatus.SUCCESS
        assert "Local artifact store accessible" in result.message
    
    def test_check_artifact_store_s3(self, basic_config):
        """S3 Artifact Store 체크."""
        basic_config["artifact_store"] = {
            "type": "s3",
            "config": {
        ...
        assert result.status == CheckStatus.SUCCESS
        assert "S3 bucket accessible" in result.message
    ...
    def test_no_preprocessed_output_check(self, basic_config):
        ...
        basic_config["output"]["preprocessed"] = {
            ...
            "adapter_type": "storage",
            "config": {
                "base_path": "./artifacts/preprocessed"
            }
        }
