...
class TestSqlAdapterSecurityValidation:
    ...
    def test_sql_security_guards_select_star_prevention(self, settings_builder):
        """Test SQL security guard prevents SELECT * queries."""
        ...
        with adapter.engine.begin() as conn:
            conn.execute(sqlalchemy.text(
                "CREATE TABLE test_security (id INTEGER, value TEXT)"
            ))
            conn.execute(sqlalchemy.text(
                "INSERT INTO test_security VALUES (1, 'data1'), (2, 'data2')"
            ))

        # When/Then: SELECT * should be prevented
        with pytest.raises(ValueError, match="SELECT \\*.*금지됩니다"):
            adapter.read("SELECT * FROM test_security")
    ...
    def test_sql_security_guards_limit_warning(self, settings_builder):
        ...
        with adapter.engine.begin() as conn:
            conn.execute(sqlalchemy.text(
                "CREATE TABLE test_limit (id INTEGER, value TEXT)"
            ))
            conn.execute(sqlalchemy.text(
                "INSERT INTO test_limit VALUES (1, 'data1'), (2, 'data2')"
            ))
...
class TestSqlAdapterErrorHandling:
    ...
    def test_connection_failure_handling(self, settings_builder):
        ...
        settings = settings_builder \
            .with_data_source("sql", config={
                "connection_uri": "postgresql://invalid:invalid@nonexistent:5432/nonexistent"
            }) \
            .build()
    ...
    def test_engine_creation_failure_handling(self, settings_builder):
        ...
        settings = settings_builder \
            .with_data_source("sql", config={
                "connection_uri": "invalid://broken/connection/string"
            }) \
            .build()
...
class TestSqlAdapterDatabaseTestContext:
    ...
    def test_sql_adapter_with_database_test_context(self, settings_builder, database_test_context, test_data_generator):
        ...
        with database_test_context.sqlite_db({"users": cls_df, "products": reg_df}) as db:
            ...
            users_df = adapter.read("SELECT feature_1, feature_2, feature_3, feature_4, entity_id, target FROM users")
            products_df = adapter.read("SELECT feature_1, feature_2, feature_3, entity_id, target FROM products")
    ...
    def test_sql_file_loading_functionality(self, settings_builder, database_test_context, test_data_generator, isolated_temp_directory):
        ...
        sql_file.write_text("SELECT feature_1, feature_2, feature_3, entity_id, target FROM test_data WHERE target = 1")
...
class TestSqlAdapterDatabaseTypeHandling:
    ...
    def test_postgresql_uri_parsing(self, settings_builder):
        ...
        settings = settings_builder \
            .with_data_source("sql", config={
                "connection_uri": "sqlite:///:memory:"  # Use SQLite for actual testing
            }) \
            .build()
        ...
        db_type, processed_uri, engine_kwargs = adapter._parse_connection_uri("postgresql://user:pass@localhost:5432/db")
    ...
    def test_mysql_uri_parsing(self, settings_builder):
        ...
        db_type, processed_uri, engine_kwargs = adapter._parse_connection_uri("mysql://user:pass@localhost:3306/db")
    ...
    def test_unknown_scheme_handling(self, settings_builder):
        ...
        db_type, processed_uri, engine_kwargs = adapter._parse_connection_uri("unknown://user:pass@localhost:1234/db")
