...
class TestRegressionDataHandler:
    ...
    def test_split_data_for_regression(self, settings_builder, test_data_generator):
        ...
        # Then: Data is split correctly
        assert len(train_df) == 80  # 80% train
        assert len(test_df) == 20   # 20% test
    ...
    def test_prepare_data_zero_variance(self, settings_builder):
        ...
        settings = settings_builder \
            ...
            .with_target_column("target") \
            .build()
        settings.recipe.data.data_interface.feature_columns = ["constant_feature", "variable_feature"]
