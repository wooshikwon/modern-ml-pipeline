"""
Timeseries Wrappers Tests (커버리지 확장)
...
class TestARIMAWrapper:
    """ARIMAWrapper 테스트 - 실제 객체 사용"""
    
    def test_arima_window_mismatch_error(self, test_data_generator):
        """케이스 A: 윈도우 길이·특성 수 불일치 시 명확한 예외"""
        # Given: ARIMA wrapper 초기화
        arima = ARIMAWrapper(order_p=1, order_d=1, order_q=1)
    ...
    def test_arima_minimal_sequence_single_step_prediction(self, test_data_generator):
        ...
        try:
            ...
            assert not np.isnan(prediction[0])  # 유효한 예측값
            ...
            error_msg = str(e).lower()
            assert any(keyword in error_msg for keyword in 
                      ['arima', 'model', 'data', 'convergence', 'optimization'])
    
    def test_arima_not_fitted_prediction_error(self):
        """보너스: 학습되지 않은 모델로 예측 시도 시 명확한 에러"""
        # Given: 학습되지 않은 ARIMA wrapper
        arima = ARIMAWrapper(order_p=1, order_d=1, order_q=1)
...
class TestExponentialSmoothingWrapper:
    """ExponentialSmoothingWrapper 테스트 - 실제 객체 사용"""
    
    def test_exponential_smoothing_minimal_sequence_prediction(self, test_data_generator):
        """최소 길이 시퀀스에서 지수평활법 예측 성공"""
        # Given: ExponentialSmoothing wrapper (트렌드만, 계절성 없음)
        exp_smooth = ExponentialSmoothingWrapper(trend="add", seasonal=None)
        ...
        try:
            ...
            assert prediction[0] > 0  # 양수 예측값 (시계열이 양수였으므로)
            ...
            error_msg = str(e).lower()
            assert any(keyword in error_msg for keyword in 
                      ['exponential', 'smooth', 'model', 'data', 'trend'])
    ...
    def test_exponential_smoothing_with_seasonal_error(self, test_data_generator):
        ...
        try:
            ...
            error_msg = str(e).lower()
            assert any(keyword in error_msg for keyword in 
                      ['seasonal', 'period', 'data', 'length', 'insufficient'])
        except Exception as e:
            # 다른 statsmodels 관련 에러들
            error_msg = str(e).lower()
            assert any(keyword in error_msg for keyword in 
                      ['exponential', 'smooth', 'seasonal', 'model'])
    
    def test_exponential_smoothing_not_fitted_error(self):
        """보너스: 학습되지 않은 모델로 예측 시도 시 에러"""
        # Given: 학습되지 않은 ExponentialSmoothing wrapper
        exp_smooth = ExponentialSmoothingWrapper(trend="add", seasonal=None)
...
class TestTimeseriesWrappersIntegration:
    ...
    def test_sklearn_interface_compatibility(self):
        """sklearn 인터페이스 호환성 확인"""
        # Given: 시계열 래퍼들
        arima = ARIMAWrapper(order_p=1, order_d=1, order_q=1)
        exp_smooth = ExponentialSmoothingWrapper(trend="add")
        
        # When & Then: sklearn 인터페이스 메서드들이 존재하는지 확인
        assert hasattr(arima, 'fit')
        assert hasattr(arima, 'predict')
        assert hasattr(exp_smooth, 'fit')
        assert hasattr(exp_smooth, 'predict')
        
        # BaseEstimator와 RegressorMixin 상속 확인
        from sklearn.base import BaseEstimator, RegressorMixin
        assert isinstance(arima, BaseEstimator)
        assert isinstance(arima, RegressorMixin)
        assert isinstance(exp_smooth, BaseEstimator)
        assert isinstance(exp_smooth, RegressorMixin)
    ...
    def test_timeseries_wrappers_module_exports(self):
        ...
        assert 'ARIMAWrapper' in timeseries_wrappers.__all__
        assert 'ExponentialSmoothingWrapper' in timeseries_wrappers.__all__
        
        # 실제 클래스들도 접근 가능한지 확인
        assert hasattr(timeseries_wrappers, 'ARIMAWrapper')
        assert hasattr(timeseries_wrappers, 'ExponentialSmoothingWrapper')
    
    def test_wrapper_parameter_validation(self):
        """래퍼 초기화 파라미터 검증"""
        # Given & When: 다양한 파라미터로 래퍼 초기화
        arima1 = ARIMAWrapper(order_p=2, order_d=1, order_q=1)
        arima2 = ARIMAWrapper(order_p=1, order_d=0, order_q=2)
        
        exp1 = ExponentialSmoothingWrapper(trend=None, seasonal=None)
        exp2 = ExponentialSmoothingWrapper(trend="add", seasonal="mul", seasonal_periods=4)
