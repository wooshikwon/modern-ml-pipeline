...
class TestCLIPipelineIntegration:
    ...
    def test_train_command_full_pipeline_execution(self, mlflow_test_context, component_test_context):
        """Test complete train workflow: CLI → SettingsFactory → TrainPipeline → MLflow"""
        with mlflow_test_context.for_classification(experiment="cli_train_full_pipeline") as mlflow_ctx:
...
"""
                ...
                result = runner.invoke(app, [
                    ...
                    '--data-path', str(mlflow_ctx.data_path)
                ])
    ...
    def test_inference_command_full_pipeline_execution(self, mlflow_test_context, component_test_context):
        """Test complete inference workflow: CLI → SettingsFactory → InferencePipeline → Results"""
        with mlflow_test_context.for_classification(experiment="cli_inference_full_pipeline") as mlflow_ctx:
            with component_test_context.classification_stack() as comp_ctx:
...
"""
    ...
    def test_system_check_integration_with_pipeline_requirements(self, isolated_temp_directory):
...
"""
        ...
        assert any(keyword in output_lower for keyword in [
            'mlflow', 'data', 'config', 'system', 'check'
...
class TestCLIErrorPropagation:
    ...
    def test_cli_error_propagation_invalid_config_file(self, isolated_temp_directory):
...
"""
        ...
        assert any(keyword in error_output for keyword in [
            'config', 'missing', 'required', 'error', 'mlflow'
        ]), f"Expected config validation error but got: {result.output}"

    def test_cli_error_propagation_missing_data_file(self, isolated_temp_directory):
...
"""
        ...
        result = runner.invoke(app, [
            ...
            '--recipe-path', str(recipe_path),
            '--config-path', str(config_path),
            '--data-path', str(nonexistent_data_path)
        ])
        ...
        error_output = result.output.lower()
        assert any(keyword in error_output for keyword in [
            'file', 'not', 'found', 'missing', 'data', 'path'
        ]), f"Expected data file error but got: {result.output}"

    @pytest.mark.parametrize("error_scenario,expected_exit_code", [
        ("invalid_model_class", 1),
        ("malformed_yaml", 1),
        ("missing_target_column", 1),
        ("insufficient_data", 1)
    ])
    def test_cli_pipeline_error_scenarios(self, error_scenario, expected_exit_code, isolated_temp_directory):
...
"""
        ...
        assert any(keyword in error_output for keyword in expected_keywords), \
...
class TestCLIFileIOIntegration:
    ...
    def test_cli_with_real_config_files(self, isolated_temp_directory):
...
"""
        ...
        runner = CliRunner()
        minimal_result = runner.invoke(app, [
            'train',
            '--recipe-path', str(recipe_path),
            '--config-path', str(minimal_config_path),
            '--data-path', str(data_path)
        ])

        # Then: Minimal config works or fails with clear error
        if minimal_result.exit_code == 0:
            assert "training" in minimal_result.output.lower() or "완료" in minimal_result.output
        else:
            # Real behavior: Minimal config might miss required fields
            error_output = minimal_result.output.lower()
            assert any(keyword in error_output for keyword in [
                'config', 'required', 'missing', 'validation'
...
class TestCLIMLflowLifecycleIntegration:
    ...
    def test_complete_mlflow_lifecycle_through_cli(self, mlflow_test_context):
        ...
        with mlflow_test_context.for_classification(experiment="cli_mlflow_lifecycle") as mlflow_ctx:
...
"""
    ...
    def test_mlflow_experiment_consistency_across_cli_commands(self, mlflow_test_context):
        ...
        with mlflow_test_context.for_classification(experiment="cli_experiment_consistency") as mlflow_ctx:
...
"""
            ...
            for i in range(3):
                ...
                if i == 0:
                    ...
                    assert train_result.exit_code == first_exit_code, \
