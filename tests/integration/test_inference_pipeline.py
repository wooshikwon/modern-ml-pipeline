...
class TestInferencePipeline:
    ...
    def test_inference_with_schema_mismatch_fails_gracefully(self, mlflow_test_context, isolated_temp_directory):
        ...
        with mlflow_test_context.for_classification("infer_schema_test") as ctx:
            ...
            mismatched_data = pd.DataFrame({
                'different_column': [1, 2, 3],
                'another_col': [0.1, 0.2, 0.3]
            })
            ...
            assert any(keyword in error_msg.lower() for keyword in 
                      ["schema", "column", "signature", "feature", "input"])
    ...
    def test_inference_with_context_params_on_static_sql_fails_with_security_error(self, mlflow_test_context):
