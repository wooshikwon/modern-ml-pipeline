...
class TestMultiComponentPreprocessingPipelines:
    ...
    def test_end_to_end_classification_preprocessing_pipeline(self, component_test_context):
        ...
        with component_test_context.classification_stack() as ctx:
            ...
            enhanced_data['categorical_feature'] = ['A', 'B', 'C'] * (len(enhanced_data) // 3) + ['A'] * (len(enhanced_data) % 3)

            # Add some missing values (not all values should be missing)
            np.random.seed(42)
            missing_idx = np.random.choice(enhanced_data.index, size=int(0.2 * len(enhanced_data)), replace=False)
            ...
            numeric_columns = [col for col in processed_data.columns if col.startswith('feature_') and processed_data[col].dtype in ['float64', 'int64']]
    ...
    def test_preprocessing_pipeline_with_tree_based_features(self, component_test_context):
        ...
        with component_test_context.classification_stack() as ctx:
            ...
            enhanced_data = raw_df.copy()
            enhanced_data['target'] = [0, 1] * (len(enhanced_data) // 2) + [0] * (len(enhanced_data) % 2)

            # Configure pipeline with tree-based feature generation
            preprocessing_config = PreprocessorConfig(steps=[
                PreprocessorStep(type="tree_based_feature_generator",
    ...
    def test_mixed_preprocessing_pipeline_integration(self, component_test_context):
        ...
        with component_test_context.classification_stack() as ctx:
            ...
            preprocessing_config = PreprocessorConfig(steps=[
                # Targeted operations
                PreprocessorStep(type="simple_imputer", columns=["feature_0"], strategy="median"),
                PreprocessorStep(type="kbins_discretizer", columns=["numeric_to_discretize"], n_bins=5),

                # Global operations
                PreprocessorStep(type="standard_scaler"),  # Apply to all applicable numeric columns
            ])
            ...
            assert 'numeric_to_discretize' not in processed_data.columns or \
                   processed_data['numeric_to_discretize'].nunique() <= 5

            # Validate standard scaling applied globally
            numeric_cols = [col for col in processed_data.columns
                          if processed_data[col].dtype in ['float64', 'int64'] and
                             processed_data[col].std() > 0]
    ...
    def test_preprocessing_pipeline_error_recovery(self, component_test_context):
        ...
        with component_test_context.classification_stack() as ctx:
            ...
            preprocessing_config = PreprocessorConfig(steps=[
                PreprocessorStep(type="simple_imputer", columns=["nonexistent_column"], strategy="mean"),  # Should skip
...
class TestPreprocessorFactoryIntegration:
    ...
    def test_preprocessor_integrates_with_adapter_and_model(self, component_test_context):
        ...
        with component_test_context.classification_stack() as ctx:
            ...
            model = ctx.model
            target_data = raw_data['target'] if 'target' in raw_data.columns else np.array([0, 1] * (len(raw_data) // 2) + [0] * (len(raw_data) % 2))
    ...
    def test_preprocessor_settings_validation_integration(self, component_test_context):
        ...
        with component_test_context.classification_stack() as ctx:
            ...
            invalid_config = PreprocessorConfig(steps=[
            ...
            # Then: Should raise appropriate error during fit
...
class TestPreprocessorDataQualityPreservation:
    ...
    def test_data_schema_consistency_through_pipeline(self, component_test_context):
        ...
        with component_test_context.classification_stack() as ctx:
            ...
            preprocessing_config = PreprocessorConfig(steps=[
                PreprocessorStep(type="standard_scaler")  # Should maintain numeric types
            ])
            ctx.settings.recipe.preprocessor = preprocessing_config
            ...
            for col in processed_data.columns:
                if col in input_data.columns and input_data[col].dtype in ['float64', 'int64']:
                    assert processed_data[col].dtype in ['float64', 'int64'], f"Column {col} type changed unexpectedly"
        ...
        with component_test_context.classification_stack() as ctx:
            ...
            for col in ['feature_0', 'feature_1']:
                if col in processed_data.columns:
                    assert not processed_data[col].isnull().any(), f"Column {col} still has missing values after imputation"
        ...
        with component_test_context.classification_stack() as ctx:
            ...
            for col in processed_data.select_dtypes(include=[np.number]).columns:
                if processed_data[col].std() > 0:  # Only for non-constant columns
                    ...
                    assert abs(processed_mean) < 0.5, f"Column {col} mean not properly centered: {processed_mean}"
                    assert abs(processed_std - 1.0) < 0.5, f"Column {col} std not properly scaled: {processed_std}"
