...
class TestCompleteServingPipelineIntegration:
    ...
    def test_complete_serving_pipeline_with_real_classification_model(self, mlflow_test_context, component_test_context):
        """
        ...
        with mlflow_test_context.for_classification("complete_serving_cls") as mlflow_ctx:
            with component_test_context.classification_stack() as comp_ctx:
                ...
                for i in range(len(test_features)):
                    ...
                    prediction_input = {k: v for k, v in sample.items()
                                      if k not in ['target', 'entity_id']}
                ...
                feature_cols = [col for col in test_features.columns if col not in ['target', 'entity_id']]
                direct_predictions = wrapped_model.predict(test_features[feature_cols].values)

                # Test first sample for consistency
                sample = test_features.iloc[0].to_dict()
                prediction_input = {k: v for k, v in sample.items() if k in feature_cols}
                api_response = client.post("/predict", json=prediction_input)
                api_prediction = api_response.json()["prediction"]

                # API and direct predictions should match (allowing for type conversion)
                assert abs(float(api_prediction) - float(direct_predictions[0])) < 1e-6
    ...
    def test_complete_serving_pipeline_with_real_regression_model(self, mlflow_test_context, component_test_context):
        """
    ...
    def test_serving_pipeline_with_batch_inference_consistency(self, mlflow_test_context, component_test_context):
        """
...
class TestAPIServerIntegrationTesting:
    ...
    def test_all_endpoints_with_real_model_integration(self, serving_test_context):
        ...
        with serving_test_context.with_trained_model("classification") as ctx:
            ...
            required_fields = ["model_uri", "model_class_path", "hyperparameter_optimization",
                             "training_methodology", "api_schema"]
            ...
            test_inputs = [
                ...
                {"feature_0": 2, "feature_1": 1, "feature_2": 1, "feature_3": 2},  # Integer inputs
                {"feature_0": 0.0, "feature_1": 0.0, "feature_2": 0.0, "feature_3": 0.0},  # Zero inputs
            ]
    ...
    def test_api_error_handling_integration(self, serving_test_context):
        ...
        with serving_test_context.with_trained_model("classification") as ctx:
            ...
            invalid_inputs = [
                ...
                {"feature_0": "not_a_number"},  # Invalid data type
                {"feature_0": None},  # Null values
            ]
    ...
    def test_api_response_consistency_under_load(self, serving_test_context):
        ...
        with serving_test_context.with_trained_model("classification") as ctx:
            ...
            def make_prediction_request():
                ...
                input_data = {"feature_0": 1.0, "feature_1": 0.5, "feature_2": 0.3, "feature_3": 0.8}
...
class TestModelLoadingAndInferenceAccuracy:
    ...
    def test_model_loading_accuracy_validation(self, mlflow_test_context, component_test_context, performance_benchmark):
        """
        ...
        with mlflow_test_context.for_classification("accuracy_validation") as mlflow_ctx:
            with component_test_context.classification_stack() as comp_ctx:
                ...
                client = MlflowClient(tracking_uri=mlflow_ctx.mlflow_uri)
                run = client.get_run(train_result.run_id)
                training_accuracy = run.data.metrics.get('test_accuracy', 0.0)
                ...
                feature_cols = [col for col in test_data.columns if col not in ['target', 'entity_id']]
                ...
                with performance_benchmark.measure_time("serving_predictions"):
                    for i in range(len(test_data)):
                        ...
                        prediction_input = {col: sample[col] for col in feature_cols}
                ...
                assert accuracy_difference < 0.15, f"Serving accuracy {serving_accuracy} differs too much from training {training_accuracy}"

                # Step 6: Performance validation
                avg_prediction_time = performance_benchmark.get_measurement("serving_predictions") / total_predictions
                assert avg_prediction_time < 0.1, f"Average prediction time {avg_prediction_time}s too slow"

                # Cleanup
                app_context.model = None
                app_context.settings = None

    def test_model_signature_consistency_validation(self, mlflow_test_context, component_test_context):
        """
        ...
        with mlflow_test_context.for_classification("signature_consistency") as mlflow_ctx:
            with component_test_context.classification_stack() as comp_ctx:
                ...
                model_version = client.get_latest_versions(
                    name=f"model_{train_result.run_id}",
                    stages=["None"]
                )[0] if client.search_registered_models(f"name='model_{train_result.run_id}'") else None

                # Test API schema endpoint matches training signature
                api_client = TestClient(app)
                schema_response = api_client.get("/model/schema")
                assert schema_response.status_code == 200
                schema_data = schema_response.json()

                assert "prediction_request_schema" in schema_data
                request_schema = schema_data["prediction_request_schema"]
                ...
                for col in feature_cols:
                    assert col in request_schema or any(col in str(field) for field in request_schema.keys())

                # Test prediction with schema-compliant input
                raw_df = comp_ctx.adapter.read(comp_ctx.data_path)
                test_features = comp_ctx.prepare_model_input(raw_df.head(1))
                prediction_input = test_features.iloc[0].to_dict()
...
class TestPerformanceAndScalabilityTesting:
    ...
    def test_serving_performance_benchmarks(self, serving_test_context, performance_benchmark):
        ...
        with serving_test_context.with_trained_model("classification") as ctx:
            ...
            test_input = {"feature_0": 1.0, "feature_1": 0.5, "feature_2": 0.3, "feature_3": 0.8}
            ...
            for _ in range(20):
                ...
                latencies.append(performance_benchmark.get_measurement(f"single_prediction_{len(latencies)-1}"))

            # Validate latency requirements
            avg_latency = sum(latencies) / len(latencies)
            max_latency = max(latencies)

            assert avg_latency < 0.1, f"Average latency {avg_latency}s exceeds 100ms threshold"
            assert max_latency < 0.2, f"Max latency {max_latency}s exceeds 200ms threshold"
            ...
            assert throughput > 50, f"Throughput {throughput} requests/sec below 50 req/sec threshold"
            ...
            for _ in range(10):
                ...
                health_latencies.append(performance_benchmark.get_measurement(f"health_check_{len(health_latencies)-1}"))

            avg_health_latency = sum(health_latencies) / len(health_latencies)
            assert avg_health_latency < 0.05, f"Health check latency {avg_health_latency}s exceeds 50ms"
    ...
    def test_serving_resource_usage_monitoring(self, serving_test_context, performance_benchmark):
        ...
        with serving_test_context.with_trained_model("classification") as ctx:
            ...
            assert success_rate > 0.90, f"Under sustained load, success rate {success_rate} dropped below 90%"
            assert total_requests == 300, "Not all requests completed"
            ...
            assert avg_response_time < 0.15, f"Under load, average response time {avg_response_time}s exceeded 150ms"
        ...
        with serving_test_context.with_trained_model("classification") as ctx:
            ...
            for extreme_input in extreme_values:
                ...
                if response.status_code == 200:
                    ...
                    assert not (prediction == float('inf') or prediction == float('-inf') or prediction != prediction)
    ...
    def test_schema_mismatch_error_handling(self, mlflow_test_context, component_test_context):
        ...
        with mlflow_test_context.for_classification("schema_mismatch") as mlflow_ctx:
            with component_test_context.classification_stack() as comp_ctx:
                ...
                schema_violations = [
                    # Wrong feature names
                    {"wrong_feature_0": 1.0, "wrong_feature_1": 0.5, "wrong_feature_2": 0.3, "wrong_feature_3": 0.8},
                    # Extra features (should be ignored or cause error)
                    {"feature_0": 1.0, "feature_1": 0.5, "feature_2": 0.3, "feature_3": 0.8, "extra_feature": 1.0},
...
class TestProductionDeploymentScenarios:
    ...
    def test_production_readiness_validation(self, serving_test_context, performance_benchmark):
        ...
        with serving_test_context.with_trained_model("classification") as ctx:
            ...
            for endpoint in endpoints_to_test:
                ...
                endpoint_time = performance_benchmark.get_measurement(f"endpoint_{endpoint.replace('/', '_')}")
                ...
                assert response.status_code in [200, 404, 422], f"Endpoint {endpoint} returned {response.status_code}"

            # Test 3: Prediction consistency under varying loads
            base_input = {"feature_0": 1.0, "feature_1": 0.5, "feature_2": 0.3, "feature_3": 0.8}
            ...
            for pred in successful_predictions:
                assert abs(float(pred) - float(baseline_prediction)) < 1e-6, "Predictions not consistent under load"
    ...
    def test_monitoring_and_observability_integration(self, serving_test_context):
        ...
        with serving_test_context.with_trained_model("classification") as ctx:
            ...
            test_inputs = [
                ...
                {"feature_0": 0.1, "feature_1": 0.2, "feature_2": 0.1, "feature_3": 0.3},
            ]
            ...
            for i, test_input in enumerate(test_inputs):
                ...
                prediction_logs.append({
                    ...
                    "model_uri": response.json().get("model_uri") if response.status_code == 200 else None
                })
            ...
            for log in successful_predictions:
                assert log["response_time"] < 1.0, f"Prediction {log['request_id']} too slow: {log['response_time']}s"
                assert log["prediction"] is not None, f"Prediction {log['request_id']} returned null"
                assert log["model_uri"] is not None, f"Prediction {log['request_id']} missing model_uri"
            ...
            for error_input in error_inputs:
                ...
                error_logs.append({
                    "input": error_input,
                    "status_code": response.status_code,
                    "error_detail": response.json().get("detail") if response.status_code != 200 else None
                })

            # Validate error handling for monitoring
            for error_log in error_logs:
                # Should return appropriate error codes
                assert error_log["status_code"] in [400, 422, 500], f"Unexpected error status: {error_log['status_code']}"

                # Should include error details for monitoring
                if error_log["status_code"] != 500:  # Internal errors may not have details
                    assert error_log["error_detail"] is not None, "Error missing details for monitoring"
    ...
    def test_graceful_degradation_under_stress(self, serving_test_context, performance_benchmark):
        ...
        with serving_test_context.with_trained_model("classification") as ctx:
            ...
            assert success_rate > 0.7, f"Under stress, success rate {success_rate} dropped below 70%"
            ...
            recovery_input = {"feature_0": 1.0, "feature_1": 0.5, "feature_2": 0.3, "feature_3": 0.8}
...
class TestServingIntegrationConfiguration:
    ...
    def test_integration_test_environment_setup(self, mlflow_test_context, component_test_context, serving_test_context):
        ...
        with mlflow_test_context.for_classification("env_setup_test") as mlflow_ctx:
            ...
            assert mlflow_ctx.experiment_name.endswith(mlflow_ctx.experiment_name.split('-')[-1])  # UUID suffix
