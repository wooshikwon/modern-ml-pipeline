...
def setup_logging(settings: "Settings"):
    ...
    # 최신 Settings 구조: settings.config.environment.name
    env_name = getattr(getattr(settings, "config", None), "environment", None)
    env_name = getattr(env_name, "name", "local")
    if env_name == "local":
    ...
    logger.info(f"로거 설정 완료. 실행 환경: {env_name}")