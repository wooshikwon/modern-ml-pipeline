...
class EnvironmentChecker:
    ...
    def check_python_version(self) -> bool:
        ...
        if current_version.major != 3:
            self.errors.append(f"Python 3.11.x가 필요합니다. 현재: Python {current_version.major}.{current_version.minor}")
            return False
            
        if current_version.minor < 11:
            self.errors.append(f"Python 3.11 이상이 필요합니다. 현재: Python {current_version.major}.{current_version.minor}")
            return False
            
        if current_version.minor == 12:
            self.warnings.append("Python 3.12는 causalml과 호환성 문제가 있을 수 있습니다. Python 3.11.x 사용을 권장합니다.")
            
        if current_version.minor > 12:
            self.warnings.append(f"Python {current_version.major}.{current_version.minor}는 테스트되지 않았습니다. Python 3.11.x 사용을 권장합니다.")
            
        logger.info(f"✅ Python 버전 확인: {current_version.major}.{current_version.minor}.{current_version.micro}")
        return True
    ...
    def check_system_compatibility(self) -> bool:
        ...
        if system_info['system'] == 'Darwin' and system_info['machine'] == 'arm64':
            self.warnings.append("Apple Silicon Mac 감지. 일부 패키지는 Rosetta 2가 필요할 수 있습니다.")
