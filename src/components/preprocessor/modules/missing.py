...
class DropMissingWrapper(BasePreprocessor, BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y=None):
        ...
        if self.axis == 'columns':
            ...
            self._dropped_columns = missing_fractions[missing_fractions > self.threshold].index.tolist()
        else:
...
class InterpolationWrapper(BasePreprocessor, BaseEstimator, TransformerMixin):
    ...
    """
    def __init__(self, method: str = 'linear', order: Optional[int] = None, 
                 limit: Optional[int] = None, columns: List[str] = None):
        """
        Args:
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.columns is not None:
            ...
            target_cols = [col for col in result.columns 
        ...
        for col in target_cols:
            try:
                if self.method in ['polynomial', 'spline'] and self.order is not None:
                    result[col] = result[col].interpolate(method=self.method, order=self.order, limit=self.limit)
                else:
