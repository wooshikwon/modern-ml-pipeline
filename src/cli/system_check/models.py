...
class CheckResult:
    ...
    BigQuery, GCS, S3 등) 연결 검사 결과를 담는 핵심 데이터 구조입니다.

    Attributes:
        is_healthy: 검사 통과 여부
        service_name: 검사 대상 서비스 이름
        message: 주요 결과 메시지
        details: 상세 정보 목록 (선택사항)
        recommendations: 문제 해결을 위한 추천 사항들 (선택사항)
        severity: 문제의 심각도 (info, warning, important, critical) (선택사항)
    ...
    severity: Optional[str] = "info"  # info, warning, important, critical
